...
export default function Home() {
  ...
  useGSAP(
    () => {
      ...
      const headings = sections.map(section => section.querySelector<HTMLElement>(".section-heading"));
      const animatedItems = sections.map(section => gsap.utils.toArray<HTMLElement>(section.querySelectorAll(".animate-item")));
      ...
      gsap.set(sections, { visibility: 'hidden', position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 0 });
      ...
      function gotoSection(index: number, direction: number) {
        ...
        if (index === 0 && fromTop && currentSection && currentOuter && currentInner) {
          ...
          tl.to([currentOuter, currentInner], {
        ...
        if (currentHeading) {
          tl.to(currentHeading, {
            ...
            duration: 1,
            ease: "power2.inOut"
          }, 0); // Inicia imediatamente junto com a transição de seção
        }
        ...
        if (items.length > 0) {
          tl.fromTo(items, {
            ...
            ease: "power2.out"
          }, 1.2); // Cascata: inicia 0.2s depois do texto
        }
  ...
  return (
    <>
      ...
      <main ref={mainRef} className="relative w-full h-svh overflow-hidden">
        <section className="slide-section z-10 w-full h-full relative">
          <HeroSection />
        </section>

        <section className="slide-section z-0">
          <div className="outer w-full h-full overflow-hidden">
            <div className="inner w-full h-full overflow-hidden relative flex justify-center items-center bg-surface-950 3xl:p-[1.666vw] p-4 md:p-8">
              <h2 className="section-heading text-gradient text-gradient-white text-center text-[clamp(1.5rem,6vw,2.25rem)] md:text-[clamp(2.25rem,4vw,3.5rem)] font-medium 3xl:text-[2.916vw]">
        ...
        <section className="slide-section z-0">
          <div className="outer w-full h-full overflow-hidden">
            <div className="inner w-full h-full overflow-hidden relative flex justify-center items-center bg-surface-100 3xl:p-[1.666vw] p-4 md:p-8">
              <h2 className="section-heading text-gradient text-gradient-surface text-center text-[clamp(1.5rem,6vw,2.25rem)] md:text-[clamp(2.25rem,4vw,3.5rem)] font-medium 3xl:text-[2.916vw]">
        ...
        <section className="slide-section z-0">
          <div className="outer w-full h-full overflow-hidden">
            <div className="inner w-full h-full overflow-hidden relative flex justify-center items-center bg-surface-950 3xl:p-[1.666vw] p-4 md:p-8">
              <h2 className="section-heading text-gradient text-gradient-white text-center text-[clamp(1.5rem,6vw,2.25rem)] md:text-[clamp(2.25rem,4vw,3.5rem)] font-medium 3xl:text-[2.916vw]">
                Construímos a <br className="md:hidden" /> tecnologia que <br className="hidden md:max-lg:block" /> faz <br className="md:hidden" /> sentido <br className="hidden lg:block" />
      ...
      <footer ref={footerRef} className="slide-section z-0">
        <div className="outer w-full h-full grid grid-rows-[1fr_auto_1fr] overflow-hidden bg-surface-950 3xl:p-[1.666vw] p-4 md:p-8 relative">
          <div
            aria-hidden
          ...
          <div className="inner w-full overflow-hidden relative flex flex-col justify-center items-center  3xl:p-[1.666vw] p-4 md:p-8 3xl:gap-[1.666vw] gap-4 md:gap-8">
            <h2 className="section-heading text-gradient text-gradient-white text-center text-[clamp(1.5rem,6vw,2.25rem)] md:text-[clamp(2.25rem,4vw,3.5rem)] font-medium 3xl:text-[2.916vw]">
